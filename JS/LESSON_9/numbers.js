//твой возраст
let myAge = 23;

//год рождения
let birthYear = 2001;

//число рождения (без нолика в начале)
let birthDay = 22;

//количество родных братьев
let numOfBrothers = 1;

//количество родных сестер
let numOfSisters = 3;

//количество человек в семье
let numOfFamilyMembers = 9;

//стоимость проезда до места обучения
let costToStudy = 10000;

//текущий год
let currentYear = 2024;

//курс доллара
let dollarToRub = 87.74;

//курс евро
let euroToRub = 95.76;

//курс биткоина
let bitcoinToRub = 5508126;

//текущий год минус год рождения
let myAge2 = currentYear - birthYear;

//текущий год минус твой возраст
let birthYear2 = currentYear - myAge;

//количество родных братьев плюс количество родных сестер
let brothersAndSisters = numOfBrothers + numOfSisters;

//курс евро умноженный на 1000
let euro1000 = euroToRub * 1000;

//курс доллара умноженный на 2.5
let quaterEagleToRub = dollarToRub * 2.5;

//курс биткоина разделенный на 10000
let crushedBitcoin = bitcoinToRub / 10000;

//стоимость проезда до места обучения разделенная на курс доллара
let ticketPriecDollar = costToStudy / dollarToRub;

//количество человек в семье минус количество братьев минус количество сестер
let familyWithoutKids = numOfFamilyMembers - brothersAndSisters;

//0 деленный на твой возраст
let zeroCrushedAge = 0 / myAge;

//35 минус твой возраст
let decreasedAge = 35 - myAge;